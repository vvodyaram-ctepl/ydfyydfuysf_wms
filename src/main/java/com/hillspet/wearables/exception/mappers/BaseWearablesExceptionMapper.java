...
import com.hillspet.wearables.common.exceptions.AbstractBaseWearablesException;
 ...
 * BaseWEARABLESException is thrown by the application This provides an abstraction &
...
public class BaseWearablesExceptionMapper implements ExceptionMapper<AbstractBaseWearablesException> {
	@Autowired
	...
	private static final Logger LOGGER = LogManager.getLogger(BaseWearablesExceptionMapper.class);

	@Override
	public Response toResponse(AbstractBaseWearablesException exception) {
		final List<WearablesError> errors = exception.getErrors();

		LOGGER.error("A type of BaseWEARABLESException has occurred with {} status, having error values of {}", exception,
				exception.getStatus(), errors);
